...
class BotInterface:
    ...
    def current_keyboard():
        ...
        keyboard.add_line()
        keyboard.add_button('Далее', color=VK_kb.PRIMARY)
        keyboard.add_line()
            ...
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                if command == 'привет':
                    ...
                    if self.params is None:
                        self.message_send(event.user_id, f'Давайте сначала поздороваемся?')
                    else:
                        ...
                        if user['is_closed']:
                            self.message_send(event.user_id,
                                            f'Встречайте {user["name"]}, vk.com/id{user["id"]}, но этот профиль закрытый!'
                                                )
                        ...
                        if len(users) >= 3:
                            ...
                            while insert_viewed(user["id"]):
                                if len(users) >= 2:
                                    ...
                                    self.message_send(event.user_id, f'Нажмите поиск еще раз!')
                            if user['is_closed']:
                                self.message_send(event.user_id,
                                                f'Встречайте {user["name"]}, vk.com/id{user["id"]}, но этот профиль закрытый!'
                                                )
                ...
                elif command == 'просмотреть избранное':
                    if self.params is None:
                        self.message_send(event.user_id, f'Давайте сначала поздороваемся?')
                    else:
